...
export default function ClientProduct({ product, related }: Props) {
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {related.map((r) => (
            <motion.div key={r.id} whileHover={{ scale: 1.02 }} className="bg-semantic-surface-card rounded-lg p-3 shadow">
              ...
              <Link href={`/shop/${r.id}`} className="mt-2 inline-block text-sm text-semantic-text-primary">View</Link>
            </motion.div>
          ))}
        </div>
      </section>
