...
function Card({ children, title, icon }: { children: React.ReactNode; title: string; icon: React.ReactNode }) {
  return (
  <motion.div whileHover={{ y: -4 }} className="bg-linear-to-br from-cream-100/80 via-pink-50/60 to-yellow-50 rounded-2xl shadow-lg p-6">
      <div className="flex items-center gap-3 mb-4">
        <div className="bg-white/60 p-2 rounded-lg">{icon}</div>
...
export default function CustomCakeShowcase() {
  ...
  function SizeCard({
    ...
    return (
      <div className="bg-white rounded-2xl shadow-md overflow-hidden">
        <div className="w-full h-40 bg-gray-100 relative flex items-center justify-center">
        ...
        <div className="p-4">
          <h3 className="font-semibold text-lg text-pink-700">{prod.name}</h3>
          <p className="text-sm text-gray-600 mt-1">{prod.description}</p>

          <div className="mt-3 text-sm text-gray-600">Order duration: <span className="font-medium text-gray-800">{duration}</span></div>

          <div className="mt-3">
            <label className="text-sm font-medium text-gray-700 block mb-2">Layers</label>
            <div className="flex gap-2 flex-wrap">
              {options.map((o, i) => (
                  <button
                    key={o.layers}
                    onClick={() => setSelectedIdx(i)}
                    className={`px-3 py-1 rounded-md text-sm border ${i === selectedIdx ? 'bg-pink-600 text-white border-pink-600' : 'bg-white text-gray-700'}`}>
            ...
            <span className="font-bold text-pink-600">{formatGhs(prod.price)}</span>
            <motion.button whileTap={{ scale: 0.95 }} onClick={() => { add({ ...prod, options: { layers: opt.layers } }, 1); toast.success(`${prod.name} added to cart`); }} className="bg-pink-600 text-white px-3 py-1 rounded-md text-sm">Add to Cart</motion.button>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <section className="max-w-6xl mx-auto px-6 py-12">
      <h2 className="text-3xl font-extrabold text-pink-700 mb-6">Custom Cake Pricelist</h2>
      ...
      {/* Square Cakes */}
      <div className="mt-10">
        <Card title="Whipped Cream Square Cakes" icon={<SliceIcon className="text-pink-600" size={22} />}>
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {squareSizes.map((s) => (
      ...
      {/* Sheet Cakes */}
      <div className="mt-10">
        <Card title="Whipped Cream Sheet Cakes" icon={<SheetIcon className="text-pink-600" size={22} />}>
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {sheetCakes.map((s) => (
              <div key={s.label} className="bg-white rounded-2xl shadow-md p-4">
                <div className="w-full h-28 bg-gray-100 relative flex items-center justify-center mb-3">
                ...
                <h4 className="font-semibold text-pink-700">{s.label}</h4>
                <p className="text-sm text-gray-600 mt-1">{s.description}</p>
                <div className="mt-4 flex items-center justify-between">
                  <span className="font-bold text-pink-600">{formatGhs(s.price)}</span>
                  <motion.button whileTap={{ scale: 0.95 }} onClick={() => { add({ ...makeProduct({ id: `sheet-${s.label}`, name: s.label, description: s.description, price: s.price }), options: {} }, 1); toast.success(`${s.label} added to cart`); }} className="bg-pink-600 text-white px-3 py-1 rounded-md text-sm">Add to Cart</motion.button>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* Cupcakes */}
      <div className="mt-10">
        <Card title="Cupcakes" icon={<CupcakeIcon className="text-pink-600" size={22} />}>
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {cupcakes.map((c) => (
              <div key={`cup-${c.qty}`} className="bg-white rounded-2xl shadow-md p-4">
                <div className="w-full h-28 bg-gray-100 relative flex items-center justify-center mb-3">
                ...
                <h4 className="font-semibold text-pink-700">{c.qty} pieces</h4>
                <p className="text-sm text-gray-600 mt-1">Freshly baked cupcakes</p>
                <div className="mt-4 flex items-center justify-between">
                  <span className="font-bold text-pink-600">{formatGhs(c.price)}</span>
                  <motion.button whileTap={{ scale: 0.95 }} onClick={() => { add({ ...makeProduct({ id: `cup-${c.qty}`, name: `${c.qty} Cupcakes`, description: 'Cupcake box', price: c.price }), options: { qty: c.qty } }, 1); toast.success(`${c.qty} cupcakes added to cart`); }} className="bg-pink-600 text-white px-3 py-1 rounded-md text-sm">Add to Cart</motion.button>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="mt-8 flex flex-col sm:flex-row items-center justify-between gap-4">
        <p className="text-sm text-gray-600">Delivery fee depends on recipient&apos;s location.</p>

        <div className="flex gap-3">
          <Link href="/contact" className="inline-flex items-center px-4 py-2 bg-pink-600 text-white rounded-md shadow-sm">Contact to Order</Link>
          <a href="https://wa.me/?text=Hello%20DishQo%20I%20want%20to%20order%20a%20custom%20cake" target="_blank" rel="noreferrer" className="inline-flex items-center px-4 py-2 border border-pink-600 text-pink-600 rounded-md">WhatsApp</a>
        </div>
      </div>
    </section>
  );
